...
function Pill({ name, value, ok }: { name: string; value: string; ok: boolean }) {
  return (
    <div className="flex items-center gap-2 rounded-full border border-white/10 bg-black/30 px-3 py-1.5">
      <span
        className={`h-1.5 w-1.5 rounded-full ${ok ? "bg-signal-lime shadow-[0_0_8px_#b6ff4a]" : "bg-signal-amber shadow-[0_0_8px_#ffb020]"}`}
      />
...
export default function ConnectionPills({
  ...
  return (
    <div className="flex flex-wrap justify-end gap-2">
      <Pill name="STREAM" value={wsOk ? "LIVE" : "RECONNECTING"} ok={wsOk} />
